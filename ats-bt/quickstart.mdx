...
<Warning>
Do **not** enable both directions at once (e.g. `PROFILES=ON 0 1 1 1 1 1 1`, with A2DP sink **and** source both on). The module becomes unstable and will reset when you open a stream. Always use one of the single-direction configurations above.
</Warning>
...
<Tabs>
  ...
  <Tab title="BLE Audio (LE Audio)">
    ...
    <Warning>
    `OPEN ... UTX` transmits audio **from** the ATS-BT, so the module must be in **transmit mode** first (`SET PROFILES=ON 0 1 0 1 1 1 1`, then `WRITE` and `RESET`). The factory default is receive mode, so a fresh module will reset on `OPEN UTX` until you switch it. See [Transmit and Receive Modes](#transmit-and-receive-modes).
    </Warning>
